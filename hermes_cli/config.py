...
def _read_windows_user_path() -> Optional[str]:
    """Best-effort read of the current user's registry PATH on Windows.

    Returns None on non-Windows or if the registry key is missing/unreadable.
    """
...
def reload_env_vars(*, include_path: bool = False) -> Dict[str, Any]:
    """Refresh Hermes-managed env into os.environ, with optional PATH refresh.

    Adds/updates vars from ~/.hermes/.env, removes vars deleted from .env,
    and returns a typed summary. When ``include_path=True``, also refreshes
    the process-local PATH from the current Windows user registry PATH
    (`HKCU\\Environment\\Path`) so new stdio children inherit the latest
    search path without requiring a full session restart.
    """
    env_vars = load_env()
    known_keys = set(OPTIONAL_ENV_VARS.keys()) | _EXTRA_ENV_KEYS
    updated: Dict[str, Any] = {"updated_dotenv": 0, "path_refreshed": False, "path_source": None}

    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            updated["updated_dotenv"] += 1
    ...
    if include_path:
        user_path = _read_windows_user_path()
        if user_path is not None:
            current = os.environ.get("PATH", "")
            os.environ["PATH"] = user_path
            updated["path_refreshed"] = True
            updated["path_source"] = "HKCU\\Environment\\Path"
            updated["path_len"] = len(user_path)
        else:
            updated["path_refreshed"] = False
            updated["path_source"] = "skipped"
...
def reload_env() -> int:
    ...
    """
    return reload_env_vars(include_path=False)["updated_dotenv"]

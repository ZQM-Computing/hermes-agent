...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def process_command(self, command: str) -> bool:
        ...
        elif canonical == "reload-env":
            args = (cmd_original or "").splitlines()[0].split()
            include_path = "--include-path" in args or "-p" in args
            refresh_mcp = "--refresh-mcp" in args or "-m" in args
            from hermes_cli.config import reload_env_vars

            summary = reload_env_vars(include_path=include_path)
            print(
                f"  Reloaded .env: {summary['updated_dotenv']} var(s) updated"
            )
            if summary.get("path_refreshed"):
                print(
                    f"  PATH refreshed from {summary.get('path_source')}"
                )
            elif (
                "path_source" in summary and summary["path_source"] == "skipped"
            ):
                print("  PATH refresh skipped")

            if refresh_mcp:
                print("  Refreshing MCP servers...")
